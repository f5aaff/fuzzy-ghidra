...
AFL_SHOW_MAP() {
    ...
    BINARY=$2
    CRASH_FILE=$3
    afl-showmap -U -m none -o "$OUTPUT_DIR" -- "$BINARY" < "$CRASH_FILE"
}
...
for crash_file in "$AFL_OUTPUT_DIR/crashes/id"*; do
  if [ -f "$crash_file" ]; then
    ...
    AFL_SHOW_MAP "$TEMP_COVERAGE_DIR/coverage_$(basename $crash_file).map" "$BINARY" "$crash_file"
  fi
done
...
for queue_file in "$AFL_OUTPUT_DIR/queue/id"*; do
  if [ -f "$queue_file" ]; then
    ...
    AFL_SHOW_MAP "$TEMP_COVERAGE_DIR/coverage_$(basename $queue_file).map" "$BINARY" "$queue_file"
  fi
done
